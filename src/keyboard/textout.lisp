...
(in-package :Lunette.Examples.Textout)
...
(defcallback WindowFunc LRESULT ((hWnd HWND)
  ...
  (cond
    ...
    ((eql msg WM_CHAR)
     (let ((hdc (GetDC hWnd))
           (str (format nil "~A" (code-char wparam))))
       (with-foreign-string (cstr str)
           (Textout hdc (* *char-offset* 10) 0 cstr (length str)))
       (incf *char-offset*)
       (ReleaseDC hwnd hdc))
     0)
...
(defun WinMain ()
  ...
  (register-class "TextoutWin" :lpfnWndProc (callback WindowFunc))

  (let ((hwnd (create-window-ex "TextoutWin" "Textout Window")))
